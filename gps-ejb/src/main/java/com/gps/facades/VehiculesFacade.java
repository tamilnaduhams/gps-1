...
public class VehiculesFacade extends AbstractFacade implements VehiculesFacadeLocal, Serializable {
    ...
    public List<VehiculeMapPosition> findVehiculesPositions() {
        //String query = "select new VehiculeMapPosition(vp, veh, max(vp.dateEnregistrement)) vmp from VehiculePosition vp, Vehicule veh where vp.numBoitier = veh.boitierAffecte.numBoitier group by vmp.vehiculePosition.numBoitier " ;
        //String query = "select vp.numBoitier, max(vp.dateEnregistrement) from VehiculePosition vp group by vp.numBoitier" ;
        //String query = "select new com.gps.dto.VehiculeMapPosition(vp, min(vp.id)) from VehiculePosition vp group by vp.numBoitier" ;
        ...
        if(ids != null && !ids.isEmpty()){
            ...
            for (Integer id : ids) {
                String queryString = "select new com.gps.dto.VehiculeMapPosition(vp, veh) from VehiculePosition vp, Vehicule veh where vp.id = " +  id.intValue() + " and veh.boitierAffecte.numBoitier = vp.numBoitier";
                VehiculeMapPosition vmp = getEntityManager().createQuery(queryString, VehiculeMapPosition.class).getSingleResult();
    ...
    public List<VehiculePosition> fetchVehiculeItinerary(Vehicule vehicule, Date dateDebut, Date dateFin){
        ...
        StringBuilder queryString =new StringBuilder("select vp from VehiculePosition vp where ");
                ...
                Query query = getEntityManager().createQuery(queryString.toString()).setParameter(1, vehicule.getBoitierAffecte().getNumBoitier());
                if (dateDebut != null) {

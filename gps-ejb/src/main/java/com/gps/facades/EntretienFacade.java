...
public class EntretienFacade extends AbstractFacade implements EntretienFacadeLocal, Serializable {
    ...
    public List<EntretienVoiture> findAllNearEntreiens() {
        ...
        List<EntretienVoiture> ev = em.createQuery("select ev from EntretienVoiture ev where ev.effectue = 0  ").getResultList();
        
        for (EntretienVoiture entretienVoiture : ev) {
            if (entretienVoiture.getDateEntretien() != null) {
                int intervalle = generalSettingsFacade.findGeneralSettingsBySettingsCode(GeneralSettingsConstants.DAYS_INTERVALLE_GS_CODE, 10);
                int days = Days.daysBetween(new DateTime(new Date()), new DateTime(entretienVoiture.getDateEntretien())).getDays();
                if (days <= intervalle) {
                    realEv.add(entretienVoiture);
                }
            }
            if (entretienVoiture.getKilometrage() != null) {
                int kilos = generalSettingsFacade.findGeneralSettingsBySettingsCode(GeneralSettingsConstants.KILOMETERS_INTERVALLE_GS_CODE, 1500);
                int intervaleKilometres = entretienVoiture.getKilometrage() - entretienVoiture.getIdVehicule().getKilometrageReel() ;

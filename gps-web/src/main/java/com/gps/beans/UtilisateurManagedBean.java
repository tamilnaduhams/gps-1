...
public class UtilisateurManagedBean implements Serializable {
    ...
    public void create() {
        if (confPassWord != null && newPassWord != null && !confPassWord.isEmpty() && !newPassWord.isEmpty() && confPassWord.equals(newPassWord)) {
            getSelectedUtilisateur().setMotPasse(encodePassword(newPassWord));
            try {
                if (checkIdentifiantUnicity(getSelectedUtilisateur().getIdentifiant()) && checkMailUnicity(getSelectedUtilisateur().getEmail()) && checkMobileUnicity(getSelectedUtilisateur().getMobile())) {
                    ...
                    JsfUtil.addSuccessMessage("L'opération de création d'un nouveau utilisateur a été effectuée avec succès !");
                }
            } catch (Exception e) {
    ...
    public void update() {
        try {
            if (checkIdentifiantUnicityUpdate(getSelectedUtilisateur()) && checkMailUnicityUpdate(getSelectedUtilisateur()) && checkMobileUnicityUpdate(getSelectedUtilisateur())) {
    ...
    public void disableUtilisateur(Utilisateur utilis) {
        try {
            ...
            if(utilis.getEnabled()) JsfUtil.addSuccessMessage("L'activation du collaborateur a été effectuée avec succès !");
            else JsfUtil.addSuccessMessage("La désactivation du collaborateur a été effectuée avec succès !");
        } catch (Exception e) {
    ...
    public Boolean checkIdentifiantUnicity(String username) throws Exception {
        if (username != null && !username.isEmpty() && getUtilisateurFacade().findUtilisateurByIdentifiant(username) != null) {
    ...
    public Boolean checkIdentifiantUnicityUpdate(Utilisateur selectedUtilisateur) throws Exception {
        if (selectedUtilisateur.getIdentifiant()!= null && !selectedUtilisateur.getIdentifiant().isEmpty()) {
            Utilisateur utilisateur = getUtilisateurFacade().findUtilisateurByIdentifiant(selectedUtilisateur.getIdentifiant());
    ...
    public Boolean checkMobileUnicityUpdate(Utilisateur selectedCollaborateur) throws Exception {
        if (selectedCollaborateur.getMobile() != null && !selectedCollaborateur.getMobile().isEmpty()) {
            List<Utilisateur> listUtilisateurs = utilisateurFacade.findUtilisateursByMobile(selectedCollaborateur.getMobile());
            if (!listUtilisateurs.isEmpty() && listUtilisateurs.get(0).getIdUtilisateur() != selectedCollaborateur.getIdUtilisateur()) {
                JsfUtil.addErrorMessage("Mobile déjà existant");
                return false;
            }
        }
    ...
    public Boolean checkMailUnicityUpdate(Utilisateur selectedCollaborateur) throws Exception {
        if (selectedCollaborateur.getEmail() != null && !selectedCollaborateur.getEmail().isEmpty()) {
            List<Utilisateur> listUtilisateurs = utilisateurFacade.findUtilisateursByEmail(selectedCollaborateur.getEmail());
            if (!listUtilisateurs.isEmpty() && listUtilisateurs.get(0).getIdUtilisateur()!= selectedCollaborateur.getIdUtilisateur()) {
                JsfUtil.addErrorMessage("Email déjà existant");
                return false;
            }
    ...
    /**
     * @param selectedUtilisateur the selectedUtilisateur to set
     */
    ...
    /**
     * @param listUtilisateur the listUtilisateur to set
     */
